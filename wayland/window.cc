...
void WaylandWindow::SetShell(ShellType type)
{
  if (type_ == type)
    return;
...
void WaylandWindow::SetShell(ShellType type,
    WaylandShellSurface* shell_parent_, unsigned x, unsigned y)
{
  if (!shell_parent_ || type != TRANSIENT)
   NOTREACHED();
  ...
  type_ = type;
  shell_surface_->UpdateShellSurface(type_, shell_parent_, x, y);
}
